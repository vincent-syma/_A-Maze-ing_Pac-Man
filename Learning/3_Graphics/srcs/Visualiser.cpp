...
void    visualise_maze(std::vector<Maze>& maze_history)
{
    ...
    while (true) {
        ...
        std::cout << "Displaying slice " << i + 1 << " of " << n << ":         \n";
...
void    visualise_map(std::vector<Map>& map_history)
{
    RawTerminal rt; // Enable raw mode for terminal input
    int connectionNumber[5] = {0, 0, 0, 0, 0};
    int n = map_history.size();
    int i = n - 1;
    if (n == 0) {
        std::cout << "No map history to display.\n";
        return ;
    }
    std::ios::sync_with_stdio(false); // Disable synchronization with C I/O for faster input
    std::cin.tie(nullptr); // Untie cin from cout to prevent flushing on input
    ...
    while (true) {
        ...
        uint16_t width = map_history[i].get_width();
        uint16_t height = map_history[i].get_height();
        std::cout << "Displaying slice " << i + 1 << " of " << n << ":         \n";
        Map map = map_history[i];
        for (uint8_t y = 0; y < height; ++y) {
            for (uint8_t x = 0; x < width; ++x) {
                Cell cell = map.get_cell(x, y);
                if (cell.status == CellType::PATH) {
                    std::cout << ANSII_BG_WHITE "  ";
                }
                else if (cell.status == CellType::WALL) {
                    connectionNumber[1]++;
                    std::cout << ANSII_BG_BLUE "  ";
                }
                else {
                    std::cout << ANSII_BG_BLACK "  ";
                }
                std::cout << ANSII_RESET;
            }
            std::cout << "    \n";
