...
void draw_map5(SDL_Renderer* renderer, const Map& map)
{
    static std::vector<SDL_Texture*> tiles;
    static std::vector<SDL_Texture*> paths;
    static bool initialized = false;

    if (!initialized)
    {
        SDL_Texture* pathsheet = load_texture(renderer, "assets/paths_1x16.png");
        if (!pathsheet) return;

        paths = split_tiles(renderer, pathsheet, 16); // 0=WALL, 1=PATH
        SDL_DestroyTexture(pathsheet);
        SDL_Texture* tilesheet = load_texture(renderer, "assets/walls3_1x14.png");
        if (!tilesheet) return;
    ...
    for (int y = 0; y < map.get_height(); ++y)
    {
        for (int x = 0; x < map.get_width(); ++x)
        {
            ...
            if ((map.get_cell(x, y).status == CellType::WALL)) {
                ...
                SDL_Texture* tex = paths[map.get_cell(x, y).path];
                SDL_RenderTexture(renderer, tex, nullptr, &dstRect);
            }
        }
    }
